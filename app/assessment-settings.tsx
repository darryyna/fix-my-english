...
function nextLevel(level: EnglishLevel): EnglishLevel {
    ...
    return ENGLISH_LEVELS[
        Math.min(index + 1, ENGLISH_LEVELS.length - 1)
        ];
}
...
export default function AssessmentSettingsScreen() {
    ...
    const [targetLevel, setTargetLevel] = useState<EnglishLevel>(
        nextLevel(assessmentResult?.suggestedLevel ?? 'B1'),
    );

    const [goal, setGoal] =
        useState<LearningGoal>('general');

    const [customGoalTopic, setCustomGoalTopic] =
        useState('');
    ...
    const trimmedCustomGoalTopic = customGoalTopic.trim();
    const isCustomTopicMissing =
        goal === 'custom' && trimmedCustomGoalTopic.length === 0;
    ...
    const handleContinue = async () => {
        ...
        try {
            await saveAssessment(
                ...
                targetLevel,
                goal,
                goal === 'custom' ? trimmedCustomGoalTopic : null,
                focus,
                dailyGoal,
            );
            ...
            console.error('saveAssessment failed:', saveError);
                    ...
                    <View style={styles.options}>
                        {ENGLISH_LEVELS.map((item) => (
                            <OptionButton
                                key={item}
                                label={item}
                                selected={targetLevel === item}
                                onPress={() => setTargetLevel(item)}
                            />
                        ))}
                    </View>
                </Card>
                ...
                <Card style={styles.card}>
                    ...
                    {goal === 'custom' && (
                        <View style={styles.customTopicContainer}>
                            <TextInput
                                value={customGoalTopic}
                                onChangeText={setCustomGoalTopic}
                                placeholder="e.g. IT interviews, medical English..."
                            ...
                            {isCustomTopicMissing && (
                                <Text style={styles.fieldError}>
                                    Describe what topic you want to focus on.
                                </Text>
                            )}
                        </View>
                    )}
